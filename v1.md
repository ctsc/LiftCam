...
- **Mobile:** Expo (React Native, TypeScript), `expo-camera`/`expo-av`/`expo-file-system` for capture and playback, **EAS Build** for cloud iOS/Android builds (solves the no-Mac constraint, still fully App Store/Play Store publishable).
- **Backend is two processes, not one:** FastAPI handles HTTP (auth, chunked upload, job status, History). A **separate worker process** runs MediaPipe / OpenCV / scoring / overlay render. Same machine is fine for v1; they must not share a process. A 45s pose/bar pipeline would stall HTTP if it ran in-process, and the user can already close the app while a job keeps running.
...
- **Queue:** no Redis, Celery, or SQS. The `uploads` row in Postgres *is* the queue. API inserts `queued`; the worker claims with `FOR UPDATE SKIP LOCKED`.
- **Storage:** Cloudflare **R2** for video bytes (S3-compatible, no AWS required). **Postgres** for all structured data (users, uploads/jobs, segments, reps). Video is never stored in Postgres.
- **Hosting:** not AWS for v1 (no EC2/ECS/Lambda/RDS). Cheap VPS or small PaaS (Hetzner, DigitalOcean, Fly, Render) for API+worker; managed Postgres outside RDS. Revisit AWS later if GPU workers, multi-region, or company procurement require it.
...
## Runtime, Data & Auth
- **History item = one `uploads` row.** That row is both the History item and the job. It owns the R2 keys, user-entered weight/reps, and status. Do not split Upload and Job into two tables. Retry reuses the same row and the same original video; it does not spawn a second job.
...
- **`uploads.status` machine:** `uploading` → `queued` → `processing` → `ready` | `failed` | `no_lift`.
  - `failed` = pipeline crash/timeout/corrupt file. Tap to retry. Original video kept.
  - `no_lift` = worker finished: no bar, bar never moved, or no lifter. Own terminal status, not `failed` and not `ready`. Results page never opens.
...
- Explicit **`failed`** (distinct from `no_lift`) for genuine pipeline crashes/timeouts/corrupted uploads: "processing failed, tap to retry," original video preserved, failures logged internally. Retry resets the same `uploads` row back to `queued`.
- **`no_lift`** fires — and the results screen never renders at all — when: the barbell is never detected, the barbell is detected but never moves, or the lifter never appears in frame.
- **Upload resilience:** chunked/resumable upload. If the connection drops mid-upload, it resumes from the last successful chunk rather than restarting from zero. Upload pauses when the app is backgrounded and resumes automatically on reopen — foreground-only in v1, no native background-session module (keeps the "thin mobile client" principle intact).
- **Push notification timing:** not requested on first app open. Asked right after the user's first upload completes, the natural moment it's actually relevant. If denied, in-app job status still reflects reality regardless — the notification is a convenience, not the source of truth.
...
Diagrams live in `/diagrams`. V1 design closeout is these five, nothing else (no separate deployment diagram, no UI wireframes):

- `diagrams/component` — Expo app, API, worker, Postgres, R2, push
- `diagrams/activity-capture-upload` — record vs camera-roll, confirm/retake, shared async path
- `diagrams/sequence-process-job` — upload → R2 → queue → worker → notify → History
- `diagrams/er-data-model` — User, Upload, Segment, Rep (no ShareLink)
- `diagrams/state-job` — `uploading` / `queued` / `processing` / `ready` / `failed` / `no_lift`

---

Product decisions resolved through Decision 55. Design-phase grilling is in progress; locked so far: two-process backend, Postgres-as-queue, no AWS, R2 + Postgres, `uploads` as History+job, first-class segments/reps, `no_lift` as its own status, two R2 objects, overlay-only OS share/download, JWT access+refresh. Still open: media URL strategy, push provider, Postgres hosting, worker stage order, diagram format.